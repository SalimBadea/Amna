...
class DeleteItemUseCase @Inject constructor(
    ...
    private val TAG = "DeleteItemUseCase"
    operator fun invoke(itemId:Int): Flow<Resource<MainResponseModel<ContactUsResponse>>> = flow {
        try {
            emit(Resource.Loading())
            val myResponse = repository.deleteItem(itemId, "DELETE")
            Log.d(TAG, "invoke: DeleteItem use case ${myResponse.isSuccessful}")
            if (myResponse.isSuccessful && myResponse.body() != null) {
                myResponse.body()?.let { response ->
                    emit(Resource.Success(response))
                }
            } else {
                Log.d(TAG, "invoke: DeleteItem use case errorBody ${myResponse.errorBody()}")
                ...
                Log.e(TAG, "invoke: Error DeleteItem use case $errorMessage")
                emit(Resource.Error(errorMessage))
            }

        } catch (e: Exception) {
            Log.e(TAG, "invoke: Exception DeleteItem use case ${e.localizedMessage}")
            emit(
                Resource.Error(
