...
class ChangePasswordUseCase @Inject constructor(
    ...
    private val TAG = "ChangePasswordUseCase"
    operator fun invoke(body: ChangePasswordBody): Flow<Resource<MainResponseModel<ContactUsResponse>>> = flow {
        try {
            emit(Resource.Loading())
            val loginResponse = repository.changePassword(body)
            Log.d(TAG, "invoke: ChangePassword use case ${loginResponse.isSuccessful}")
            if (loginResponse.isSuccessful && loginResponse.body() != null) {
                loginResponse.body()?.let { response ->
                    emit(Resource.Success(response))
                }
            } else {
                Log.d(TAG, "invoke: ChangePassword use case errorBody ${loginResponse.errorBody()}")
                ...
                Log.e(TAG, "invoke: Error ChangePassword use case $errorMessage")
