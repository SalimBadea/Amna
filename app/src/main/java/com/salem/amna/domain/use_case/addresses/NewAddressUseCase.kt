...
class NewAddressUseCase @Inject constructor(
    ...
    private val TAG = "NewAddressUseCase"
    operator fun invoke(addressBody: AddressBody): Flow<Resource<MainResponseModel<ContactUsResponse>>> = flow {
        try {
            emit(Resource.Loading())
            val myResponse = repository.addNewAddress(addressBody)
            Log.d(TAG, "invoke: NewAddress use case ${myResponse.isSuccessful}")
            if (myResponse.isSuccessful && myResponse.body() != null) {
                ...
                Log.e(TAG, "invoke: Error NewAddress use case $errorMessage")
                emit(Resource.Error(errorMessage))
            }

        } catch (e: Exception) {
            Log.e(TAG, "invoke: Exception NewAddress use case ${e.localizedMessage}")
            emit(
                Resource.Error(
