...
class GetGovernorateUseCase @Inject constructor(
    val repository: AddressesRepository
) {

    private val TAG = "GetGovernorateUseCase"

    operator fun invoke():Flow<Resource<MainResponseModel<GovernoratesResponse>>> = flow {
        try {
            emit(Resource.Loading())

            val myResponse = repository.getGovernorates()
            Log.d(TAG, "invoke: GetGovernorate use case ${myResponse.isSuccessful}")
            if (myResponse.isSuccessful && myResponse.body() != null) {
                myResponse.body()?.let { response ->
                    emit(Resource.Success(response))
                }
            } else {
                Log.d(TAG, "invoke: GetGovernorate use case errorBody ${myResponse.errorBody()}")
                val errorString = myResponse.errorBody()?.byteStream()?.bufferedReader()
                    .use { it?.readText() }  // defaults to UTF-8

                val errorMessage =
                    getErrorResponse(errorString!!).message ?: ""
                Log.e(TAG, "invoke: Error GetGovernorate use case $errorMessage")
                emit(Resource.Error(errorMessage))
            }

        }catch (e: Exception) {
            Log.e(TAG, "invoke: Exception GetGovernorate use case ${e.localizedMessage}")
