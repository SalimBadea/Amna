...
class WithdrawalToBankAccountUseCase @Inject constructor(
    private val repository: PointsRepository
) {

    private val TAG = "WithdrawBankAccUseCase"

    operator fun invoke(body: WithdrawalToBankAccountBody): Flow<Resource<MainResponseModel<ContactUsResponse>>> = flow {
        try {
            emit(Resource.Loading())

            val myResponse = repository.withdrawalToBankAccount(body)
            Log.d(TAG, "invoke: WithdrawBankAcc use case ${myResponse.isSuccessful}")
            if (myResponse.isSuccessful && myResponse.body() != null) {
                myResponse.body()?.let { response ->
                    emit(Resource.Success(response))
                }
            } else {
                Log.d(TAG, "invoke: WithdrawBankAcc use case errorBody ${myResponse.errorBody()}")
                val errorString = myResponse.errorBody()?.byteStream()?.bufferedReader()
                    .use { it?.readText() }  // defaults to UTF-8

                val errorMessage =
                    getErrorResponse(errorString!!).message ?: ""
                Log.e(TAG, "invoke: Error WithdrawBankAcc use case $errorMessage")
                emit(Resource.Error(errorMessage))
            }

        }catch (e: Exception) {
            Log.e(TAG, "invoke: Exception WithdrawBankAcc use case ${e.localizedMessage}")
