...
class ContactUsUseCase @Inject constructor(
    ...
    private val TAG = "ContactUsUseCase"
    operator fun invoke(contactUsBody: ContactUsBody): Flow<Resource<MainResponseModel<ContactUsResponse>>> = flow {
        try {
            emit(Resource.Loading())
            val myResponse = repository.contactUs(contactUsBody)
            Log.d(TAG, "invoke: ContactUs use case ${myResponse.isSuccessful}")
            if (myResponse.isSuccessful && myResponse.body() != null) {
                myResponse.body()?.let { response ->
                    emit(Resource.Success(response))
                }
            } else {
                Log.d(TAG, "invoke: ContactUs use case errorBody ${myResponse.errorBody()}")
                ...
                Log.e(TAG, "invoke: Error ContactUs use case $errorMessage")
                emit(Resource.Error(errorMessage))
            }

        } catch (e: Exception) {
            Log.e(TAG, "invoke: Exception ContactUs use case ${e.localizedMessage}")
            emit(
                Resource.Error(
                    e.localizedMessage?:""
                )
